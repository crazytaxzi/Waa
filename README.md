...
WAA resolves the real Windows Downloads known folder when possible, including redirected corporate profiles. On startup, while the app is being used, and when **Scan Downloads Now** is pressed, WAA checks the eight most recent idle reports oldest-first so an existing four-week history becomes usable immediately. Missing BOL still uses only its newest report. Valid sources are copied into `%LOCALAPPDATA%\Waa\reports`; originals remain untouched. A lightweight file signature skips unchanged history scans, and SHA-256 content checks prevent duplicate imports.
