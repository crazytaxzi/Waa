...
function Set-WaaLiveCallField {
    param(
        ...
        [AllowNull()][string]$IdleSnapshotBasis
    )
    $key = Get-WaaLiveCallKey $DriverId $CycleKey
    $call = Get-WaaLiveCall $DriverId $CycleKey
    if ($null -eq $call) {
        $now=(Get-Date).ToUniversalTime().ToString('s')
        $call=[pscustomobject]@{id=$null;driver_id=$DriverId;cycle_key=$CycleKey;opened_at=$now;updated_at=$now;fuel_status='Unknown';fuel_note=$null;driver_eta=$null;eta_status='Unknown';eta_note=$null;idle_plan=$null;idle_percent_snapshot=$null;idle_period_end_snapshot=$null;idle_snapshot_basis=$null;load_help_status='Unknown';load_help_note=$null;conversation_wrap=$null;completed_at=$null;_revision=0;_deleted=$false}
    }
    foreach($property in @('idle_percent_snapshot','idle_period_end_snapshot','idle_snapshot_basis')){
    ...
    if($Field-eq'idle_plan'-and$CaptureIdleSnapshot){
        ...
        $call.idle_snapshot_basis=$IdleSnapshotBasis
    }
...
function Add-WaaLiveCheckpointSql {
    ...
    elseif($EntityKey.StartsWith('call:')){
        $columns=@('driver_id','cycle_key','opened_at','updated_at','fuel_status','fuel_note','driver_eta','eta_status','eta_note','idle_plan','idle_percent_snapshot','idle_period_end_snapshot','idle_snapshot_basis','load_help_status','load_help_note','conversation_wrap','completed_at')
        $values=@($columns|ForEach-Object{Get-WaaLiveSqlValue $Record $_})
