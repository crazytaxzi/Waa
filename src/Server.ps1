...
function Test-WaaClientDisconnect {
    ...
    while ($null -ne $exception) {
        if ($exception -is [IO.IOException] -or $exception -is [Net.Sockets.SocketException] -or $exception -is [ObjectDisposedException]) { return $true }
        $message = [string]$exception.Message
        if ($message -match '(?i)(transport connection|connection.*aborted|forcibly closed|broken pipe|connection reset|disposed object)') { return $true }
...
function Send-Json { param($Stream,[int]$Code,$Data) $json=$Data|ConvertTo-Json -Depth 20 -Compress; return Send-Response $Stream $Code 'application/json; charset=utf-8' ([Text.Encoding]::UTF8.GetBytes($json)) }
function Send-JsonArray { param($Stream,[int]$Code,$Data) $json=ConvertTo-Json -InputObject @($Data) -Depth 20 -Compress; return Send-Response $Stream $Code 'application/json; charset=utf-8' ([Text.Encoding]::UTF8.GetBytes($json)) }
...
for($candidate=8765;$candidate -le 8775;$candidate++){
    ...
    catch{if($candidate -eq 8775){throw}}
}
            ...
            try {
                if($path.StartsWith('/api/')){
                    ...
                    if(-not [string]::IsNullOrWhiteSpace($request.body)){
                        ...
                        if($null -ne $object){foreach($property in $object.PSObject.Properties){$body[$property.Name]=$property.Value}}
                    }
                    if($method -eq 'GET' -and $path -eq '/api/health'){Send-Json $request.stream 200 @{ok=$true;integrity=$state.integrity;read_only=$state.read_only}|Out-Null}
                    elseif($method -eq 'GET' -and $path -eq '/api/dashboard'){Send-Json $request.stream 200 (Get-Dashboard)|Out-Null}
                    elseif($method -eq 'GET' -and $path -eq '/api/drivers'){Send-JsonArray $request.stream 200 @(Get-CurrentDrivers)|Out-Null}
                    elseif($method -eq 'GET' -and $path -match '^/api/drivers/(\d+)$'){Send-Json $request.stream 200 (Get-DriverCard ([int]$Matches[1]))|Out-Null}
                    elseif($method -eq 'POST' -and $path -match '^/api/drivers/(\d+)/action$'){Send-Json $request.stream 200 (Save-DriverAction ([int]$Matches[1]) $body)|Out-Null}
                    elseif($method -eq 'GET' -and $path -match '^/api/drivers/(\d+)/conversation$'){Send-Json $request.stream 200 (Get-WaaConversation -DriverId ([int]$Matches[1]))|Out-Null}
                    elseif($method -eq 'POST' -and $path -match '^/api/drivers/(\d+)/conversation$'){Send-Json $request.stream 200 (Save-WaaConversation -DriverId ([int]$Matches[1]) -Body $body)|Out-Null}
                    ...
                    elseif($method -eq 'POST' -and $path -eq '/api/import/commit'){
                        if($body.type -and $body.type -ne 'pta'){throw 'Rolling 7-Day and Missing BOL reports are managed automatically from Downloads. PTA is paste-only.'}
                        Send-Json $request.stream 201 (Import-WaaData ([string]$body.raw) 'PTA paste' 'pta')|Out-Null
                    }
                    elseif($method -eq 'GET' -and $path -eq '/api/report-intake'){Send-Json $request.stream 200 (Get-WaaReportIntakeStatus)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/report-intake/scan'){$scan=Invoke-WaaDownloadsScan;$script:LastIntakeScan=Get-Date;Send-Json $request.stream 200 $scan|Out-Null}
                    elseif($method -eq 'GET' -and $path -eq '/api/data-quality'){Send-Json $request.stream 200 (Get-DataQuality)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/identity/resolve'){Send-Json $request.stream 200 (Resolve-Identity ([int]$body.issue_id) ([int]$body.driver_id))|Out-Null}
                    elseif($method -eq 'GET' -and $path -eq '/api/transition'){Send-Json $request.stream 200 (Get-Transition)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/transition/regenerate'){Send-Json $request.stream 200 (Get-Transition -Regenerate)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/transition'){Send-Json $request.stream 200 (Save-Transition $body.body)|Out-Null}
                    elseif($method -eq 'GET' -and $path -eq '/api/safety/random'){$except=0;if($uri.Query -match 'except=(\d+)'){$except=[int]$Matches[1]};Send-Json $request.stream 200 (Get-SafetyNote $except)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/backup'){Send-Json $request.stream 201 (Backup-Waa)|Out-Null}
                    elseif($method -eq 'POST' -and $path -eq '/api/restore'){Send-Json $request.stream 200 (Restore-Waa $body.name)|Out-Null}
                    ...
                    else{Send-Json $request.stream 404 @{error='API route not found'}|Out-Null}
                }
                else {
                    ...
                    if(-not $file.StartsWith($web,[StringComparison]::OrdinalIgnoreCase) -or -not (Test-Path $file -PathType Leaf)){Send-Json $request.stream 404 @{error='Not found'}|Out-Null}
                    else {
